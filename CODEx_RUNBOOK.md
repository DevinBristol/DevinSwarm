...
- **Active work items:**
  - [ ] Add filters to `/ui` (state/phase/iteration/runId) and surface durations per node/event for observability.
  - [ ] Add backfill script for assignee fields to update existing runs and document it in Render/local steps.
  - [ ] Make retry limits (plan/dev/review/ops) and maxIterations configurable via env and surface in runbook/UI.
  - [ ] Add tests covering reservations and event payload shape (iteration, duration, assignee).
  - [ ] Harden reviewer/ops prompts with risk/deploy checklists (platform stubs) and ensure iteration policy reiterated.
  - [ ] (Optional) Pin `msgpackr` to 1.11.5 until upstream prepare is fixed (Cloud/CI still use `ci:install` to skip prepare).  ✅ (done)
