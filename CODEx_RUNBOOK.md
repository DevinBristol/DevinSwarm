...
- **Active work items:**
  - [x] Define and encode self-iteration policy (when to replan vs HITL vs fail) in this runbook and in prompts.
  - [x] Instrument event stream/UI to emit iteration metadata and reasons and surface them in `/ui` (run table + events include iteration/maxIterations, assignees).
  - [x] Harden manager/dev/reviewer/ops prompts to respect iteration limits and self-iteration policy; add checklists for reviewer/ops.
