...
- **Runtime note:** `scripts/run-ts-node.js` (ts-node transpile-only) backs start/test scripts to bypass `tsx`/esbuild spawn failures on Windows; npm prints a harmless `npm-prefix` warning after scripts.
